...
exports.getAllFurniture = async (req, res) => {
  try {
    ...
    let query = db.collection('furniture_items');
...
//      const data = doc.data();
//      if ((!minPrice || data.price >= parseFloat(minPrice)) &&
...
exports.getFurnitureById = async (req, res) => {
  try {
    const doc = await db.collection('furniture_items').doc(req.params.id).get();
...
exports.searchFurniture = async (req, res) => {
  try {
    ...
    const snapshot = await db.collection('furniture_items').get();
    const items = [];
...
exports.getFeaturedFurniture = async (req, res) => {
  try {
    const snapshot = await db.collection('furniture_items')
      .where('featured', '==', true)
      .limit(10)
      .get();
