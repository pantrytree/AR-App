...
const projectValidation = {
  createProject: [
    body('name').trim().notEmpty().withMessage('Project name is required').isLength({ min: 3, max: 100 }).withMessage('Project name must be between 3 and 100 characters'),
    body('description').trim().notEmpty().withMessage('Project description is required').isLength({ min: 10, max: 1000 }).withMessage('Description must be between 10 and 1000 characters'),
...
const designValidation = {
  createDesign: [
    ...
    body('name').trim().notEmpty().withMessage('Design name is required').isLength({ min: 3, max: 100 }).withMessage('Design name must be between 3 and 100 characters'),
    body('canvasData').optional().isObject().withMessage('Canvas data must be an object'),
    validate,
  ],

  updateDesign: [
    param('id').notEmpty().withMessage('Design ID is required'),
    body('name').optional().trim().isLength({ min: 3, max: 100 }).withMessage('Design name must be between 3 and 100 characters'),
    body('canvasData').optional().isObject().withMessage('Canvas data must be an object'),
    validate,
  ],
...
const designObjectValidation = {
  addDesignObject: [
    ...
    body('furnitureItemId').notEmpty().withMessage('Furniture item ID is required'),
    ...
    body('position.z').optional().isFloat().withMessage('Position z must be a number'),
    body('rotation').optional().isObject().withMessage('Rotation must be an object'),
    body('scale').optional().isObject().withMessage('Scale must be an object'),
    validate,
  ],

  updateDesignObject: [
    param('id').notEmpty().withMessage('Object ID is required'),
    body('position').optional().isObject().withMessage('Position must be an object'),
    body('rotation').optional().isObject().withMessage('Rotation must be an object'),
    body('scale').optional().isObject().withMessage('Scale must be an object'),
    validate,
...
const furnitureValidation = {
  createFurniture: [
    body('name').trim().notEmpty().withMessage('Furniture name is required').isLength({ min: 3, max: 100 }).withMessage('Name must be between 3 and 100 characters'),
  ...
  getFurniture: [
    param('id').notEmpty().withMessage('Furniture ID is required'),
    validate,
  ],

  searchFurniture: [
    query('query').optional().trim().isLength({ min: 2 }).withMessage('Search query must be at least 2 characters'),
    validate,
  ],
};
...
const favoriteValidation = {
  ...
  removeFavorite: [
    param('furnitureItemId').notEmpty().withMessage('Furniture item ID is required'),
    validate,
  ],
...
const collaborationValidation = {
  ...
  updateRole: [
    param('id').notEmpty().withMessage('Collaboration ID is required'),
    body('role').notEmpty().withMessage('Role is required').isIn(['viewer', 'editor', 'admin']).withMessage('Role must be viewer, editor, or admin'),
    validate,
  ],
...
const isValidEmail = (email) => { const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; return emailRegex.test(email); }; // Validates email format using regex
const isValidUrl = (url) => { try { new URL(url); return true; } catch (error) { return false; } }; // Validates URL format using URL constructor
const sanitizeString = (str) => { return str.trim().replace(/[<>]/g, ''); }; // Removes potentially dangerous characters from strings
const validateObjectId = (id) => { return id && id.length > 0; }; // Basic ID presence validation
